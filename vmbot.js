...
const { Jupiter, routePriceCalculator } = pkg;
...
class VBMMBot {
    constructor() {
        ...
        this.isEmergencyStopped = false; // Renamed to avoid shadowing emergencyStop method
        this.tradeQueue = [];
        this.metrics = {
            tradesExecuted: 0,
            volumeTraded: 0,
    ...
    async initializeJupiter() {
        try {
            this.jupiter = new Jupiter({
                ...
                wrapUnwrapSOL: true,
                user: this.wallets[0]?.keypair || Keypair.generate() // Use first wallet or fallback
            });
