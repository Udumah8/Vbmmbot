...
const JUPITER_API_BACKUP = 'https://api.jup.ag/v6'; // Fallback
...
class VBMMBot {
    ...
    async initializeJupiter() {
        try {
            ...
            const httpAgent = proxy ? new SocksProxyAgent(proxy) : null;
            this.jupiter = {
                computeRoutes: async ({ inputMint, outputMint, amount, slippageBps, feeBps, onlyDirectRoutes }) => {
                ...
                swap: async ({ routeInfo, userPublicKey, wrapUnwrapSOL }) => {
            ...
            console.log('✅ Jupiter HTTP client initialized');
            if (proxy) {
                this.updateProxyStatus(proxy, true);
            }
        } catch (error) {
            console.error('❌ Jupiter initialization failed:', error.message, error.stack);
            if (proxy) {
                this.updateProxyStatus(proxy, false);
            }
            throw error;
        ...
        try {
            ...
            for (const [index, walletConfig] of walletsData.entries()) {
                try {
                    ...
                    try {
                        privateKeyBuffer = Buffer.from(decryptedPrivateKey, 'base64');
    ...
    async executeVolumeBoostTrade(inputToken, outputToken, amount) {
        ...
        const wallet = this.getNextWallet('volume_boost', amount / LAMPORTS_PER_SOL * 1.1);
        if (!wallet) {
        ...
        while (retries < MAX_RETRIES) {
            try {
                ...
                const swapResponse = await this.jupiter.swap({
                    route: bestRoute,
    ...
    async getRealMarketData(inputToken, outputToken) {
        try {
            ...
            const routes = quoteResponse.data || [];
...
async healthCheck() {
    try {
        ...
        // Check Jupiter API
        const proxy = this.getNextProxy(); // Properly fetch proxy
        const httpAgent = proxy ? new SocksProxyAgent(proxy) : null;
        try {
            await this.jupiter.computeRoutes({
                inputMint: new PublicKey(SOL_MINT),
            ...
            console.warn(`⚠️  Jupiter health check failed: ${jupiterError.message}`);
            if (proxy) {
                this.updateProxyStatus(proxy, false);
            }
        }
    ...
    async emergencyStop() {
        if (this.isEmergencyStopped) {
            console.log('ℹ️  Emergency stop already activated');
            return;
        }
    ...
    async emergencyShutdown() {
        ...
        try {
            if (this.connection) {
                console.log('🛑 Closing RPC connection...');
                this.connection = null;
            }
            if (this.jupiter) {
                console.log('🛑 Closing Jupiter client...');
                this.jupiter = null;
            }
    ...
    getMetrics() {
        ...
        return {
            status: this.isEmergencyStopped ? 'EMERGENCY_STOP' : (this.isRunning ? 'Running' : 'Stopped'),
    ...
    async startTrading(strategyConfig) {
        ...
        try {
            while (this.isRunning && !this.isEmergencyStopped) {
                try {
    ...
    decrypt(text) {
        try {
            if (!text.includes(':')) {
                throw new Error('Invalid privateKey format: missing IV separator (:)');
            }
            ...
            if (!result) {
                throw new Error('Decrypted output is empty');
            }
            return result;
        } catch (error) {
    ...
    program
        ...
        .action(async (options) => {
            try {
                ...
                console.log('\nNext: Save ENCRYPTION_KEY in .env, fund wallets, and run "vbmm-bot init -c config/trading-config.json"');
            } catch (error) {
